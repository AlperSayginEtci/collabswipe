...
export function MentionTextarea({ value, onChange, placeholder, className, autoFocus }: MentionTextareaProps) {
  ...
  const handleInput = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (match) {
      ...
      setCursorPos(selectionEnd - match[1].length - 1); // position of the @
      
      // Calculate basic position
      // In a real robust implementation, we'd use a library like get-caret-coordinates
      // For now, let's just place it near the bottom of the textarea
  ...
  const handleSelectMention = (user: any) => {
    ...
    // Insert the name directly, perhaps bolding it later if we parse mentions
  ...
  return (
    <div className="relative w-full">
      ...
      {mentionQuery !== null && (
        <div 
          className="absolute z-50 bg-card border border-border shadow-xl rounded-xl w-64 max-h-60 overflow-y-auto overflow-x-hidden"
