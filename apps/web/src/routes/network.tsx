          ...
          <div key={item.follower.id} className="flex items-center justify-between p-3 hover:bg-muted/40 rounded-xl transition-colors">
            <Link to="/profile" search={{ userId: item.follower.id }} className="flex items-center gap-3 flex-1 min-w-0">
              <div className="w-12 h-12 rounded-full bg-secondary overflow-hidden shrink-0 border border-border/40">
                <img src={(item.follower?.image || ((item.follower as any)?.role === 'company' ? `https://ui-avatars.com/api/?name=%F0%9F%92%BC&background=e2e8f0&color=94a3b8&size=1024` : 'https://www.gravatar.com/avatar/00000000000000000000000000000000?d=mp&f=y&s=1024'))} alt={item.follower.name || ''} className="w-full h-full object-cover" />
              </div>
        ...
        {tab === 'following' && followingData?.map((item) => (
          <div key={item.following.id} className="flex items-center justify-between p-3 hover:bg-muted/40 rounded-xl transition-colors">
            <Link to="/profile" search={{ userId: item.following.id }} className="flex items-center gap-3 flex-1 min-w-0">
              <div className="w-12 h-12 rounded-full bg-secondary overflow-hidden shrink-0 border border-border/40">
                <img src={(item.following?.image || ((item.following as any)?.role === 'company' ? `https://ui-avatars.com/api/?name=%F0%9F%92%BC&background=e2e8f0&color=94a3b8&size=1024` : 'https://www.gravatar.com/avatar/00000000000000000000000000000000?d=mp&f=y&s=1024'))} alt={item.following.name || ''} className="w-full h-full object-cover" />
              </div>
              <div className="min-w-0">
          ...
          return (
            <div key={user.id} className="flex items-center justify-between p-3 hover:bg-muted/40 rounded-xl transition-colors">
              <Link to="/profile" search={{ userId: user.id }} className="flex items-center gap-3 flex-1 min-w-0">
                <div className="w-12 h-12 rounded-full bg-secondary overflow-hidden shrink-0 border border-border/40">
                  <img src={(user?.image || ((user as any)?.role === 'company' ? `https://ui-avatars.com/api/?name=%F0%9F%92%BC&background=e2e8f0&color=94a3b8&size=1024` : 'https://www.gravatar.com/avatar/00000000000000000000000000000000?d=mp&f=y&s=1024'))} alt={user.name || ''} className="w-full h-full object-cover" />
                </div>
                <div className="min-w-0">
        ...
        {tab === 'connectionRequests' && targetId === loggedInUserId && connectionRequestsData?.map((item) => (
          <div key={item.requester.id} className="flex items-center justify-between p-3 hover:bg-muted/40 rounded-xl transition-colors">
            <Link to="/profile" search={{ userId: item.requester.id }} className="flex items-center gap-3 flex-1 min-w-0">
              <div className="w-12 h-12 rounded-full bg-secondary overflow-hidden shrink-0 border border-border/40">
                <img src={(item.requester?.image || ((item.requester as any)?.role === 'company' ? `https://ui-avatars.com/api/?name=%F0%9F%92%BC&background=e2e8f0&color=94a3b8&size=1024` : 'https://www.gravatar.com/avatar/00000000000000000000000000000000?d=mp&f=y&s=1024'))} alt={item.requester.name || ''} className="w-full h-full object-cover" />
              </div>
              <div className="min-w-0">
